...
import FloatingSettingButton from './FloatingSettingButton';
...
const SettingLayout: React.FC<Props> = ({ children }:Props) => {
  ...
  return (
    <>
      <div>{children}</div>
      {showChatBox? <SettingDialog position={position}  onClose={() => setShowChatBox(false)} /> :<FloatingSettingButton onToggleChat={handleChatButtonClick} position={position} setPosition={setPosition}/>}
